...
In order to assess model performance, I used k-fold cross validation on the model selected by both-direction stepwise selection. K-fold cross validation starts by splitting the dataset into k equally sized "folds". Then, for each unique group, 1 fold is held out as the test dataset whereas the other remaining folds combine to form the training dataset. For values of of k between 2 and 20 inclusive, it was determined that k = 11 folds produced the best results. These showed that the model has an r-squared value of 0.393. While this is fairly low for an r-squared model value, it does show that there is at least some model fit from the data. This is supported by the low root mean squared error (RMSE) value which implies good model strength.

```{r k-fold cv, echo = FALSE, include = TRUE}
set.seed(1234)
train_control <- trainControl(method = "cv", number = 11)
modelpred <- train(Score ~ Tees + CourseLength + Holes + CourseType + Temperature + 
                     WindSpeed + StepsWalked, data = udisc, method = "glm", 
...
It was also interesting to observe the predictors that were not deemed statistically significant when it came to having an expected effect on my score. These predictors are the month when I played the round (`Month`), the nearest hour to when I began my round (`Hour`), the type of weather I was playing in (`Weather`), the amount of time I played for (`TimePlayed`), and the number of guests I was playing the round with (`Guests`). It appears that initial presumptions that the weather had little effect on my scoring spread, as it was dropped by the stepwise model and the most significant factor, drizzle, only had had an r-squared value of 0.171. It is also intriguing to observe the fact that the number of guests is not a significant predictor of round score, also being dropped by the stepwise model and having an r-squared value of 0.013.

Even though this analysis was just done on my play alone, some of these conclusions could have applications for disc golfers and play in general that are worth discussing. The first of these is that player fatigue is a real thing on long courses with ample walking. Disc golf is typically considered on the more leisurely side but for peak performance it seems to pay to be in shape and avoid fatigue on the course where rounds can take upwards of 3 hours on courses over 2 miles in length. This is especially true for tournaments where 2 full rounds are often played within 1 day. I've learned that it will be important to consider this moving forward and expend energy only when necessary. The second is that weather most definitely has an impact on play and is super important to consider when scheduling events. Many of my worst rounds in recent memory have come when the average winds were at 15 miles per hour or above, so it is important to minimize my time in these conditions as much as possible. It will be worth it to schedule future tournaments or key practice rounds during days where winds are low and temperatures are mild to warm. The final main conclusion is that people definitely have preferences when it comes to course type. Someone like me who favors the forehand throw may score better at wooded courses where accuracy is required off the tee and distances are not as far. However, someone who may be more backhand prevalent could find more success at an open course where scoring is done through distance and power and accuracy receives less punishment.

While this report uncovered many conclusions that were interesting and unexpected, I did experience some limitations that may have impacted results or conclusions. A main shortcoming of my approach was using multiple linear regression as the base for my full model. Although this is not necessarily a bad choice for modeling a numerical response variable with varied numerical and categorical predictor variables, it was clear that one or more variables had some nonlinear correlation with score that may not have appeared in the results. One other area I could have excelled more in was looking further into hole-specific data in order to find my weak points at that level, but I was not able to compile this data quick enough for proper assessment. 

# Appendix

## Appendix A: Full Model Output (Multiple Linear Regression)
